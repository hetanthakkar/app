...
var pic, temp, temp1, temp2;
var publics = [];
var newpublics = [];
export default class ListAvatarExample extends Component {
  componentDidMount() {
    console.log("bar ma" + this.props.currentId);
    for (let k = 0; k <= 15; k++) {
      firebase
        .database()
        .ref("user/" + k)
  ...
  id = (item) => {
    for (let j = 0; j < this.state.city.length; j++) {
      if (item == this.state.dis[j]) temp2 = j;
    }
    return this.state.id[temp2];
  };
  ...
  onChange = async (bar) => {
    await this.setState({ bar: bar.toLowerCase() });
    if (bar.length != 0) {
      let dis = [];
      let disp = [];
      let about = [];
